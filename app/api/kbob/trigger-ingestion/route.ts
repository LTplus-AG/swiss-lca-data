...
export async function POST(request: Request) {
  try {
    ...
    const kbobUrl = await kv.get<string>(KBOB_URL_KEY) || "https://backend.kbob.admin.ch/fileservice/sdweb-docs-prod-kbobadminch-files/files/2024/12/03/f331074f-1f32-485a-8028-1705895cca48.xlsx";
    ...
    await sendSlackNotification(`✅ KBOB data ingestion completed successfully!\n• ${materials.length} materials processed\n• Timestamp: ${timestamp}\n• URL: ${kbobUrl}`);
    ...
    await sendSlackNotification(`❌ KBOB data ingestion failed!\n• Error: ${errorMessage}`);
